...
import necklace2 from "./../assets/images/necklace3.jpeg";
...
const VirtualTryOn = () => {
    ...
    const loadResources = async () => {
      try {
        ...
        if (selectedNecklace !== null) {
          textureLoader.load(selectedNecklace, (texture) => {
            texture.colorSpace = THREE.SRGBColorSpace;
            const geometry = new THREE.PlaneGeometry(2, 1);
            const material = new THREE.MeshBasicMaterial({
              map: texture,
  ...
  useEffect(() => {
    const detectAndPositionAccessories = async () => {
      if (!webcamRef.current || !model || (!glassesMesh && !necklaceMesh)) return;
    ...
    >
      ...
      >
        <div
          ...
          onClick={() => handleAccessorySelection("glasses", null)}
          style={{
        ...
        <div
          className="h-12 w-12 rounded-full relative"
          onClick={() => handleAccessorySelection("necklace", null)}
          style={{
